...
"""
Chúng ta sử dụng framework pytorch, trong torchvision.datasets có chứa dataset COCO
"""
...
class DatasetCOCO(CocoDetection):
    def __init__(self,root_path,year,mode,transform = None):
        annFile = os.path.join(root_path,'annatations','instances_{}{}.json'.format(mode,year))
        root = os.path.join(root_path,'{}{}'.format(mode,year))
        super(DatasetCOCO,self).__init__(root,annFile)
    ...
    def __getitem__(self, item):
        image, target = super(DatasetCOCO, self).__getitem__(item)
