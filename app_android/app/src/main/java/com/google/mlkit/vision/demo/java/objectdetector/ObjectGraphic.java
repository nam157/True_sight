...
public class ObjectGraphic extends Graphic {
    ...
    private static final String LABEL_FORMAT = "%.2f%% độ chính xác (stt: %d)";
    ...
    public ObjectGraphic(GraphicOverlay overlay, DetectedObject object, double distance) {
        super(overlay);
    ...
    public void draw(Canvas canvas) {
        ...
        float textWidth = textPaints[colorID].measureText("Khoảng cách: " + this.distance + "mm");
        ...
        canvas.drawText(
                //"ID vật thể: " + object.getTrackingId(),
                "khoảng cách: " + String.format("%.2f", this.distance) + " m",
                rect.left,
                rect.top + yLabelOffset,
                textPaints[colorID]);
        ...
        for (Label label : object.getLabels()) {
            String label_t = util.labelProcessing(label.getText());

            canvas.drawText(label_t, rect.left, rect.top + yLabelOffset, textPaints[colorID]);
            yLabelOffset += lineHeight;
            canvas.drawText(
